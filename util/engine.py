...
def train_one_epoch_acc(
    ...
    for i in range(len(data_loader)):
        with accelerator.accumulate(model):
            ...
            losses = sum(loss for name, loss in loss_dict.items() if name.startswith("loss"))
...
def append_epoch_metrics(output_dir, epoch, train_metric_logger, coco_evaluator, accelerator=None):
    if accelerator is not None and not accelerator.is_main_process:
        return

    metric_names = ["AP", "AP50", "AP75", "APS", "APM", "APL"]
    metric_names += ["AR1", "AR10", "AR100", "ARS", "ARM", "ARL"]
    val_metrics = dict(zip(metric_names, map(float, coco_evaluator.coco_eval["bbox"].stats)))
    train_metrics = {
        k.replace("loss_", "loss/"): float(v)
        for k, v in metric_logger_to_dict(train_metric_logger).items()
    }
    record = {
        "epoch": int(epoch),
        "train": train_metrics,
