...
class SalienceTransformerDecoderLayer(nn.Module):
    ...
    def set_decoder_position(self, layer_idx, num_layers):
        self.layer_idx = layer_idx
        self.num_decoder_layers = num_layers
        enable_gsda = self.use_gsda and layer_idx >= self.gsda_start_layer
        enable_bbcr = self.use_bbcr and layer_idx >= max(num_layers - self.bbcr_layers, 0)
        self.cross_attn.use_gsda = enable_gsda
        self.cross_attn.use_bbcr = enable_bbcr
...
class SalienceTransformerDecoder(nn.Module):
    def __init__(self, decoder_layer, num_layers, num_classes):
        ...
        for layer_idx, layer in enumerate(self.layers):
            if hasattr(layer, "set_decoder_position"):
                layer.set_decoder_position(layer_idx, num_layers)
